<template>
	<div class="table">
		...
		<template v-for="(quote, index) in sellQuote" :key="'quote-sell-' + index">
			<div class="table-content-row table-content-row-body table-content-row-body-sell" :data-index="'sell-' + index" v-if="index <= 7" :class="{'new-price': differentSellPrice.indexOf(sellPrice[index]) != '-1'}">
				<p class="table-content table-content-price" :data-index="'sell-price-' + index" @mouseover="hover(index, 'sell', $event)" @mouseleave="leave('sell', $event)">{{ formatSellPrice[index] }}</p>
				<p class="table-content table-content-size" :data-index="'sell-size-' + index" @mouseover="hover(index, 'sell', $event)" @mouseleave="leave('sell', $event)" :class="{'size-increase': compareSize(index, sellQuote, sameSellPriceQuote, 'increase'), 'size-decrease': compareSize(index, sellQuote, sameSellPriceQuote, 'decrease')}">{{ quote.size }}</p>
				<div class="table-content table-content-total" :data-index="'sell-total-' + index" @mouseover="hover(index, 'sell', $event)" @mouseleave="leave('sell', $event)">
					{{ sellTotal[index] }}
					<div class="tooltip">
		...
		<template v-for="(quote, index) in buyQuote" :key="'quote-buy-' + index">
			<div class="table-content-row table-content-row-body table-content-row-body-buy" :data-index="'buy-' + index" v-if="index <= 7" :class="{'new-price': differentBuyPrice.indexOf(buyPrice[index]) != '-1'}">
				<p class="table-content table-content-price" :data-index="'buy-price-' + index" @mouseover="hover(index, 'buy', $event)" @mouseleave="leave('buy', $event)">{{ formatBuyPrice[index] }}</p>
				<p class="table-content table-content-size" :data-index="'buy-size-' + index" @mouseover="hover(index, 'buy', $event)" @mouseleave="leave('buy', $event)" :class="{'size-increase': compareSize(index, buyQuote, sameBuyPriceQuote, 'increase'), 'size-decrease': compareSize(index, buyQuote, sameBuyPriceQuote, 'decrease')}">{{ quote.size }}</p>
				<div class="table-content table-content-total" :data-index="'buy-total-' + index" @mouseover="hover(index, 'buy', $event)" @mouseleave="leave('buy', $event)">
					{{ buyTotal[index] }}
					<div class="tooltip">
						<p>Avg Price: {{ formatAvgPrice }} USD</p>
						<p>Total Value: {{ formatTotalValue }} USD</p>
